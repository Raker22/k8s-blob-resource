...
import (
	...
	"github.com/raker22/k8s-blob-resource/pkg/apis/blob/v1alpha1"
)
...
func (t *TestHandler) ObjectCreated(obj interface{}) {
	...
	// assert the type to a Blob object to pull out relevant data
	blob := obj.(*v1alpha1.Blob)
	log.Infof("    ResourceVersion: %s", blob.ObjectMeta.ResourceVersion)
	log.Infof("    Name: %s", blob.ObjectMeta.Name)
}
